#!/usr/bin/env python3
"""
Modified SQLite CUPTI analyzer for NUMA experiment files.

This is a fixed version of analyze_latency.py that handles the specific
naming convention used in our NUMA locality experiments and fixes type errors.
"""
...
class SQLiteCUPTIAnalyzer:
    ...
    def parse_configuration(self) -> Dict[str, Any]:
        """Extract configuration details from filename - updated for NUMA experiment naming."""
        config = {
            'gpu_id': 'unknown',
            'numa_node': 'unknown',
        ...
        # Pattern for NUMA experiment files: gpu{N}_numa{N}_{model_name}
        numa_pattern = r'gpu(\d+)_numa(\d+)_(.+)'
        match = re.search(numa_pattern, self.config_name)
        ...
        if match:
            ...
            # Map model names to ViT configurations
            model_configs = {
                'noop': {'depth': 0, 'patch_size': 32, 'dim': 256, 'type': 'NOOP (Memory Only)'},
                'light_small': {'depth': 2, 'patch_size': 32, 'dim': 128, 'type': 'Light Small ViT'},
                'light_medium': {'depth': 4, 'patch_size': 32, 'dim': 256, 'type': 'Light Medium ViT'},
                'medium_balanced': {'depth': 6, 'patch_size': 32, 'dim': 512, 'type': 'Medium Balanced ViT'},
                'medium_compute': {'depth': 8, 'patch_size': 16, 'dim': 768, 'type': 'Medium Compute ViT'},
                'heavy_large': {'depth': 12, 'patch_size': 16, 'dim': 1024, 'type': 'Heavy Large ViT'}
            }
            
            model_config = model_configs.get(model_name, {
                'depth': 'unknown', 'patch_size': 'unknown', 'dim': 'unknown', 
                'type': f'Unknown Model: {model_name}'
            })
            
            config.update({
                'gpu_id': gpu_id,
                'numa_node': numa_node,
                'vit_patch_size': model_config['patch_size'],
                'vit_depth': model_config['depth'],
                'vit_dim': model_config['dim'],
                'test_type': model_config['type']
            })
    ...
    def analyze_pipeline_gaps_detailed(self) -> Dict[str, Any]:
        """Enhanced gap analysis with distribution info - fixed for type errors."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            time_condition, time_params = self.get_time_conditions()
...
def write_analysis_results(results: Dict[str, Any], output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        if results.get('configuration'):
            ...
            if config.get('vit_depth', 0) != 'unknown' and config.get('vit_depth', 0) > 0:
        ...
        if results.get('pipeline_analysis') and 'error' not in results['pipeline_analysis']:
            ...
            f.write("\n")
        elif results.get('pipeline_analysis', {}).get('error'):
            f.write("PIPELINE ANALYSIS\n")
            f.write("-" * 20 + "\n")
            f.write(f"Error: {results['pipeline_analysis']['error']}\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="SQLite CUPTI performance analyzer for NUMA experiments.",

...
class HydraExperimentRunner:
    ...
    def _build_command(self, run_config: Dict[str, Any]) -> List[str]:
        ...
        # Resolve all configuration overrides (vit_config overrides test_config)
        warmup_samples = vit_cfg.get('warmup_samples') or test_cfg['warmup_samples']
        batch_size = vit_cfg.get('batch_size') or test_cfg['batch_size'] 
        compile_model = vit_cfg.get('compile_model', test_cfg.get('compile_model', False))
        compile_mode = vit_cfg.get('compile_mode') or test_cfg.get('compile_mode', 'default')

        # Build base command
        cmd = ["python3", "gpu_numa_pipeline_test.py"]

        # Core arguments
        cmd.extend(["--gpu-id", str(run_config['gpu_id'])])
        cmd.extend(["--shape"] + [str(x) for x in test_cfg['tensor_shape']])
        cmd.extend(["--num-samples", str(test_cfg['num_samples'])])
        cmd.extend(["--batch-size", str(batch_size)])
        cmd.extend(["--warmup-samples", str(warmup_samples)])
        cmd.extend(["--memory-size-mb", str(test_cfg['memory_size_mb'])])

        # ViT model arguments
        cmd.extend(["--vit-patch-size", str(vit_cfg['patch_size'])])
        cmd.extend(["--vit-depth", str(vit_cfg['depth'])])
        cmd.extend(["--vit-heads", str(vit_cfg['heads'])])
        cmd.extend(["--vit-dim", str(vit_cfg['dim'])])
        cmd.extend(["--vit-mlp-dim", str(vit_cfg['mlp_dim'])])
